...
def reciprocal(market: Market, verbose: bool = False) -> dict:
    """Reciprocal matching algorithm that ranks jobs for each candidate based on the product of the candidate's and job's preferences.
    ...
    pref_prod = market.pref_cand * market.pref_job.T
    rec = np.array(
        [
            sorted(
                [j for j in range(market.num_job)],
                key=lambda j: pref_prod[i, j],
                reverse=True,
    ...
    if verbose:
        print(
            f"Reciprocal: SW = {match_probability.sum():.5f}, Time = {end_time - start_time:.5f} [s]"
        )
